...
def recursive_floyd_warshall(outer_loop:int, middle_loop:int, inner_loop:int):
        ...
        if inner_loop >= MAX_LENGTH:
            return recursive_floyd_warshall(outer_loop,
                middle_loop + 1,
                MIN_LEVEL
                                            )
        ...
        if middle_loop == inner_loop:
            ...
            start_to_end = GRAPH[middle_loop][inner_loop]

            through_intermediate = (GRAPH[middle_loop][outer_loop]  + GRAPH[outer_loop][inner_loop]
            )

            GRAPH[middle_loop][inner_loop] = min(start_to_end, through_intermediate)


            # Increase the inner loop.
